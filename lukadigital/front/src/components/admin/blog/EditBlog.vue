             ...
             ABRIR MENU
              ...
              <b-row>
               <div class="content-painel content-visitas">
                <h3 class="text-center titlesAdmin">Edição de postagem</h3>
                   <load v-if="loading"></load>
                  <p>
                      <h3 class="titleCadastro">Geral</h3>
                    </p>
                                         <div class="row">
                              ...
                              <div class="col-md-12">
                                <br>
                                <img v-if="!updateMain_image" v-bind:src="urlApi+globalForm.main_img" style="" />
                                <img v-else v-bind:src="globalForm.main_img" style="" />
                              </div>
                    </div>
                    <br>
                       ...
                       <div class="row">
                         ...
                         <div class="row">
                          <div class="blogImagesText">
                                     ...
                                     <img v-for="(imagem,index) in globalForm.imgs" v-on:click="insertImage(imagem)" v-bind:src="urlApi+imagem" class="img-responsive img-blog_upload" />
                ...
                <br><br>
                  <p class="pull-right">
                  <a v-if="requisicao_feita == 0"id="botaoLoginAdmin" v-on:click="enviaForm()" class="button botaoPadraoAdmin">Atualizar</a>
               </p>
               <div class="alert alert-success" v-if="message">Cadastrado com sucesso!</div>
...
export default {
  ...
  data () {
    return {
      ...
      message: false,
      type : '',
      updateMain_image: false
      
    }
  },

  mounted(){
      this.blog('selectPost_edit',this.$route.params.id);
  ...
  methods:{
        enviaForm: function(){
            ...
            delete this.globalForm._id;
            var aux = this.globalForm.content;
            this.globalForm.content = aux.replace(/["]/g, "'");
          if(this.erro.qtd_error == 0)
                this.blog('updatePost',this.$route.params.id,this.globalForm,this.updateMain_image);  
        },
        showMenu: function(){
    ...
    recebeData(){
     this.$refs.tm.editor.setContent(this.globalForm.content)
    },
      uploadFoto_main(){
        this.updateMain_image = true;
         this.type = 'main'
