            ...
            ABRIR MENU
              ...
              <b-row>
               <div class="content-painel content-visitas">
                <h3 class="text-center titlesAdmin">Edição de usuario</h3>
                <content-placeholders  v-if="loading">
                  ...
                  <div class="ajustContainer container" v-else>
                   <div class="row">
                            ...
                            <div v-if="in_array_group('Corretor')">
                                                          <div class="row">
                     ...
                     <br>
                            ...
                            <div  v-if="globalForm.person_type == 'F'" class="row">
                                ...
                                <div class="col-md-4 col-xs-12 form-linha">
                                  ...
                                  <select v-bind:class="{'erroInput': erro.pf_data.nacionalidadeId}" v-model="globalForm.pf_data.nacionalidadeId" name="nacionaAlugar" class="selectPadraoAdmin">
                                       <option  v-for="nac in nacionalidades" :value="nac.id" :selected="nac.name == 'BRASILEIRO' ? 'selected' : ''">{{nac.name}}</option>
                            ...
                            <div v-else class="row">
                                ...
                                <div class="col-md-4 col-xs-12 form-linha">
                                 ...
                                 <br>
                       <div  v-if="globalForm.person_type == 'F'" class="row">
                                <div v-if="!in_array_group('Corretor')" class="col-md-8 col-xs-12 form-linha">
                                    <label for="profissaoAlugar">Profissão:</label>
                                     <multiselect v-model="profissaoId" :options="profissao" :custom-label="nameWithLang" placeholder="Select one" label="name" track-by="name"></multiselect>
                                        ...
                                        <div class="row" >
                                          <div class="col-md-3">
                                            <label for="">Inicio</label>
                                              <select v-model="globalForm.horarios.hora_inicio" type="text" class="selectPadraoAdmin" name="ramoAtivi" value="">
                                             ...
                                             <div class="col-md-3">
                                              <label for="">Fim</label>
                                                <select v-model="globalForm.horarios.hora_fim" type="text" class="selectPadraoAdmin" name="ramoAtivi" value="">
                                                 <option v-for="hour in consomeJSON.horarios()" v-bind:key="hour.id" v-bind:value="hour">{{hour}}</option>        
                                                </select>
                                          </div>
                                          <div class="col-md-3">
                                            <label for="">Almoço inicio</label>
                                              <select v-model="globalForm.horarios.almoco_inicio" type="text" class="selectPadraoAdmin" name="ramoAtivi"  value="">
                                       ...
                                       <option v-for="hour in consomeJSON.horarios()" v-bind:key="hour.id" v-bind:value="hour">{{hour}}</option>
                                          ...
                                          <div class="col-md-3">
                                            <label for="">Almoço fim</label>
                                              <select v-model="globalForm.horarios.almoco_fim" type="text" class="selectPadraoAdmin" name="ramoAtivi"  value="">>
                                                <option v-for="hour in consomeJSON.horarios()" v-bind:key="hour.id" v-bind:value="hour">{{hour}}</option>
                                              </select>
                                          </div>
                                        </div>
                              ...
                              <repeat v-for="hour in dias" v-bind:key="hour.id" style="margin-left: 1em;">
                                <input v-model="globalForm.horarios.diaSemana" type="checkbox" v-bind:id="hour.name" v-bind:value="hour.id"/>
                            ...
                            <div class="row">
                              ...
                              <div class="col-md-4 col-xs-12 form-linha">
                                     <label for="cidaAlugar">Cidade:</label>
                                        <multiselect v-model="cidadeId" :options="cidades" :custom-label="nameWithLang" placeholder="Selecione um" label="name" track-by="name"></multiselect>
                                        <br>
                           ...
                           <div class="row" v-if="changePass">
                              ...
                              <div class="col-md-6 col-xs-12 form-linha">
                                     <label for="confirmpassAlugar">Confirma a senha:</label>
                                                             <input v-model="confirma_senha"  type="text" class="inputPadraoAdmin" name="confirmpassAlugar" id="confirmpassAlugar" value=""> 
                              </div>
                           ...
                           <p class="pull-right">
                           <a v-on:click="enviaForm()" id="botaoLoginAdmin" class="button botaoPadraoAdmin">Atualizar</a>
                         </p>
...
export default {
  ...
  mounted(){
    this.admin('selectSingleUser',this.$route.params.id);
            var main = this;
    ...
    enviaForm: function(){
          var aux = [];
           for(var key in this.tipoCadastro){
            ...
            this.globalForm.schedule = this.globalForm.horarios;
            this.globalForm.group_id = aux;
               ...
               if(this.changePass){
                   if(this.confirma_senha == this.globalForm.password ){
                        ...
                        this.admin('updateUser',this.globalForm,this.$route.params.id);
                     }else{
   ...
   recebeAjax(tipo){
          if(tipo == 'cep'){
                     ...
                     for(var key in this.cidades){
                          if(this.cidades[key].id.toLowerCase() == this.resposta.cidade.toLowerCase()){
                 ...
                 }else if(tipo == 'dadosUser'){
                    var aux = {};
                    aux.id = this.globalForm.pf_data.profissaoId;
                    aux.name =  this.consomeJSON.getProfissaoByID(aux.id);

                    this.profissaoId.push(aux);

                    aux = [];
                    var count = 0;
                    for(var key in this.globalForm.group_id){
                        aux[count] = {};
                        aux[count].value =  this.globalForm.group_id[key];
                        aux[count].name = this.validForm.getLabel_users(this.globalForm.group_id[key]).all;
                        count++;
                    }
                    this.tipoCadastro = aux;
                    this.trocaCity();
                    this.cidadeId = {};
                    this.cidadeId.id = this.globalForm.addresses.cidade;
                    this.cidadeId.name = this.consomeJSON.getCidadeByID(this.globalForm.addresses.uf, this.globalForm.addresses.cidade);
                   // console.log(this.cidadeId.name);
                 }
    },
